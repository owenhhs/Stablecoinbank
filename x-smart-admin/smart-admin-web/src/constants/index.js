/*
 * 所有常量入口
 *
 * @Author:    1024创新实验室-主任：卓大
 * @Date:      2022-09-06 19:58:28
 * @Wechat:    zhuda1024
 * @Email:     lab1024@163.com
 * @Copyright  1024创新实验室 （ https://1024lab.net ），Since 2012
 */
import menu from './system/menu-const';
import { LOGIN_DEVICE_ENUM } from './system/login-device-const';
import { FLAG_NUMBER_ENUM, GENDER_ENUM, USER_TYPE_ENUM } from './common-const';
import { LAYOUT_ENUM } from './layout-const';
import file from './support/file-const';
import notice from './business/oa/notice-const';
import loginLog from './support/login-log-const';
import enterprise from './business/oa/enterprise-const';
import codeGeneratorConst from './support/code-generator-const';
import changeLogConst from './support/change-log-const';
import orderInfoConst from '/@/constants/business/erp/order-info-const.js';
import collectionCardConst from '/@/constants/business/erp/collection-card-const.js';
import skuCfgConst from '/@/constants/business/erp/sku-cfg-const.js';
import merchantManageConst from '/@/constants/business/marchant-manage/merchant-manage-const.js';
import settlementInfoConst from './business/erp/settlement-info-const';
import merchantCardManageConst from './business/merchant-card-manage/merchant-card-manage-const';

export default {
  FLAG_NUMBER_ENUM,
  LOGIN_DEVICE_ENUM,
  GENDER_ENUM,
  USER_TYPE_ENUM,
  LAYOUT_ENUM,
  ...loginLog,
  ...menu,
  ...file,
  ...notice,
  ...enterprise,
  ...codeGeneratorConst,
  ...changeLogConst,
  ...orderInfoConst,
  ...collectionCardConst,
  ...skuCfgConst,
  ...merchantManageConst,
  ...settlementInfoConst,
  ...merchantCardManageConst,
};
