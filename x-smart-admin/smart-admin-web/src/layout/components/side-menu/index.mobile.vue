...
<template>
  ...
  <div class="menu">
    ...
    <van-sidebar v-model="activeMenu" style="width: 100%">
      <van-sidebar-item v-for="menu in mobileMenuTree" :title="menu.menuName" :key="menu.menuNameEn" @click="turnToPage(menu)" />
    </van-sidebar>
  </div>
</template>
...
<script setup>
  ...
  const mobileMenuTree = computed(() => {
    ...
    // 简易处理：只展示商家订单
    const merchantMenu = menuTree.value.find((menu) => menu.menuNameEn === 'Channel Order Manage');
    const { children } = merchantMenu || {};
    const merchantOrderMenu = children?.find((menu) => menu.menuNameEn === 'Payment Order');
    if (merchantOrderMenu) {
      defaultMenus.push(merchantOrderMenu)
    }
    return defaultMenus;
  });
  ...
  onMounted(() => {
    const { path } = route;
    const menuIndex = mobileMenuTree.value.findIndex((menu) => menu.path === path);
    if (menuIndex === -1) {
      activeMenu.value = 0;
      turnToPage(mobileMenuTree.value[activeMenu.value]);
      return;
    }
