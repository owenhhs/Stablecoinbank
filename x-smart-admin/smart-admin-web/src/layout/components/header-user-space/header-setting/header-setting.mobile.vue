<template>
  <van-popup v-model:show="visible" position="right" :style="{ width: '70%', height: '100%' }" @close="close">
    <div class="logo">
      <div class="title smart-logo title-light" v-if="sideMenuTheme === 'light'">{{ $t('setting.title') }}</div>
      <div class="title smart-logo title-dark" v-if="sideMenuTheme === 'dark'">{{ $t('setting.title') }}</div>
    </div>
    ...
    <van-popup v-model:show="showLanguagePicker" position="bottom">
      <van-picker :columns="i18nList" @cancel="showLanguagePicker = false" @confirm="confirmLanguageSelect" />
    </van-popup>
...
<script setup>
  ...
  const languageText = computed(() => i18nList.find((item) => item.value === formState.language)?.text);

  const confirmLanguageSelect = (options) => {
    const [value] = options.selectedValues;
