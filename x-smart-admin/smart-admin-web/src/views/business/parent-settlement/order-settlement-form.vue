  ...
  <a-modal :title="$t('settle.orderSettleForm.title')" :width="300" :open="visibleFlag" @cancel="onClose" :maskClosable="false" :destroyOnClose="true">
    ...
    <template #footer>
      <a-space>
        ...
        <a-button type="primary" @click="onSubmit" v-privilege="'settle:parent:exchange:set'">{{ $t('business.form.confirm') }}</a-button>
      </a-space>
    </template>
  </a-modal>
</template>
<script setup>
  ...
  async function postSubmit() {
    ...
    SmartLoading.show();
    try {
      await settlementInfoApi.parentExchangeRate(params);
      emits('reloadList');
      onClose();
    } catch (err) {
