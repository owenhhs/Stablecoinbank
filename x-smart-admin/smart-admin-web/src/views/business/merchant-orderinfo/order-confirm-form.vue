...
<template>
  <a-modal title="订单确认" :width="600" :open="visibleFlag" @cancel="onClose" :maskClosable="false" :destroyOnClose="true">
    ...
    <div class="tip">
      是否收到来自于
      <span class="tip--highlight">{{ form.depositHolder }}</span>
      的
      <span class="tip--highlight">{{ form.bankAccount }}</span>
      卡到账
      <span class="tip--highlight">{{ form.amount }}</span>
      元
    </div>
    <a-divider />
    <template #footer>
      <a-space>
        ...
        <a-button v-if="form.status === 1 || form.status === 4" type="primary" @click="onSubmit">{{ $t('business.form.confirm') }}</a-button>
