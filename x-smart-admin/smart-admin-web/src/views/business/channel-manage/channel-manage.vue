  ...
  <a-card size="small" :bordered="false" :hoverable="true">
    ...
    <a-row class="smart-table-btn-block">
      <div class="smart-table-operate-block">
        <a-button @click="showForm(null, false)" type="primary" size="small" v-privilege="'channel:info:add'">
          <template #icon>
            <PlusOutlined/>
          </template>
    ...
    >
      <template #bodyCell="{ text, record, column }">
        <template v-if="column.dataIndex === 'paymentScale'">
        ...
        <template v-if="column.dataIndex === 'blackList'">
          <span>{{ $smartEnumPlugin.getDescByValue('MER_CARD_BLACK_LIST', text) }}</span>
        </template>

        <template v-if="column.dataIndex === 'routeType'">
          <span>{{ $smartEnumPlugin.getDescByValue('MER_CARD_ROUTE_TYPE', text) }}</span>
        </template>

        <template v-if="column.dataIndex === 'action'">
          <div class="smart-table-operate">
            <a-button @click="showForm(record, true)" type="link" >{{ $t('business.form.view') }}</a-button>
            <a-button @click="showForm(record, false)" type="link" v-privilege="'channel:info:update'" >{{ $t('business.form.edit') }}</a-button>
          </div>
        </template>
      </template>
    </a-table>
...
import ChannelInfoForm from "/src/views/business/channel-manage/channel-info-form.vue";
...
const columns = ref([
  ...
  {
    title: t('channelManage.form.phone'),
    dataIndex: 'phone',
    ellipsis: true,
  },
  {
    title: t('channelManage.form.paymentLimit'),
    dataIndex: 'paymentLimit',
    ellipsis: true,
  },
  {
    title: t('channelManage.form.paymentCount'),
    dataIndex: 'paymentCount',
    ellipsis: true,
  },
  {
    title: t('channelManage.form.amountMin'),
    dataIndex: 'amountMin',
    ellipsis: true,
  },
  {
    title: t('channelManage.form.amountMax'),
    dataIndex: 'amountMax',
    ellipsis: true,
  },
  {
    title: t('channelManage.form.blackList'),
    dataIndex: 'blackList',
    ellipsis: true,
  },
  {
    title: t('channelManage.form.routeType'),
    dataIndex: 'routeType',
    ellipsis: true,
  },
  {
...
async function queryData() {
  ...
  try {
    let queryResult = await paymentChannelInfoApi.queryChannelInfoList();
