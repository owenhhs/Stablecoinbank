...
echo '构建前端服务'
docker build --memory 1024m --memory-swap 4g --no-cache --rm -t x-smart-admin-web:"$tag" --build-arg DEPLOY="$build_env" .
echo '前端服务构建完成'

echo '上传文件到对象存储'

docker create --name temp-container-"$tag" x-smart-admin-web:"$tag"
docker cp temp-container-"$tag":/usr/share/nginx/html /tmp/container-fs-"$tag"
coscli cp /tmp/container-fs-"$tag"/ cos://payment-"$env"-1327269739/admin-web/static/ -r
