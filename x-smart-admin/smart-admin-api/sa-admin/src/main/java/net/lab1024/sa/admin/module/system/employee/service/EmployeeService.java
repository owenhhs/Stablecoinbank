...
public class EmployeeService {
    ...
    public ResponseDTO<PageResult<EmployeeVO>> queryEmployee(EmployeeQueryForm employeeQueryForm) {
        ...
        employeeList.forEach(e -> {
            ...
            if (e.getEnabledOtpFlag() == null) {
                e.setEnabledOtpFlag(false);
            }
        });
        PageResult<EmployeeVO> pageResult = SmartPageUtil.convert2PageResult(pageParam, employeeList);
        return ResponseDTO.ok(pageResult);
    ...
    public synchronized ResponseDTO<String> addEmployee(EmployeeAddForm employeeAddForm) {
        ...
        List<Long> departmentIdList = departmentService.selfAndChildrenIdList(requestUser.getDepartmentId());
        Set<Long> departmentIdSet = new HashSet<>(departmentIdList);
        if (!departmentIdSet.contains(employeeAddForm.getDepartmentId())) {
            return ResponseDTO.userErrorParam("没有该部门权限");
        }
        ...
        employeeManager.saveEmployee(entity, employeeAddForm.getRoleIdList(), employeeAddForm.getEnabledOtpFlag());
    ...
    public synchronized ResponseDTO<String> updateEmployee(EmployeeUpdateForm employeeUpdateForm) {
        ...
        List<Long> departmentIdList = departmentService.selfAndChildrenIdList(requestUser.getDepartmentId());
        Set<Long> departmentIdSet = new HashSet<>(departmentIdList);
        if (!departmentIdSet.contains(employeeUpdateForm.getDepartmentId())) {
            return ResponseDTO.userErrorParam("没有该部门权限");
        }
        ...
        // 更新数据
        employeeManager.updateEmployee(entity, employeeUpdateForm.getRoleIdList(), employeeUpdateForm.getEnabledOtpFlag());
    ...
    public ResponseDTO<String> batchUpdateDepartment(EmployeeBatchUpdateDepartmentForm batchUpdateDepartmentForm) {
        ...
        List<Long> departmentIdList = departmentService.selfAndChildrenIdList(requestUser.getDepartmentId());
        Set<Long> departmentIdSet = new HashSet<>(departmentIdList);
        if (!departmentIdSet.contains(batchUpdateDepartmentForm.getDepartmentId())) {
            return ResponseDTO.userErrorParam("没有该部门权限");
        }
     ...
     * 重置员工多因子
     */
    public void resetEmployeeOtp(Long employeeId) {
        EmployeeOtpEntity employeeOtpEntity = employeeOtpManager.getEmployeeOtp(employeeId);
        if (employeeOtpEntity == null) {
            return;
        }
        employeeOtpEntity.setSecretKey(OtpUtl.generateSecretKey());
        employeeOtpEntity.setBoundFlag(false);
