...
import net.lab1024.sa.admin.module.system.login.domain.LoginOtpVerifyForm;
...
public class LoginController {
    ...
    @Operation(summary = "获取多因子绑定二维码")
    @GetMapping("/login/otp/qrocde")
    @NoNeedLogin
    public ResponseDTO<String> getEmployeeOtpQrcode(@RequestParam String scrip) {
        return loginService.getEmployeeOtpQrcode(scrip);
    }

    @Operation(summary = "多因子绑定验证")
    @PostMapping("/login/otp/bind")
    @NoNeedLogin
    public ResponseDTO<String> bindEmployeeOtp(@RequestBody @Valid LoginOtpVerifyForm bindForm) {
        return loginService.bindEmployeeOtp(bindForm);
    }

    @Operation(summary = "多因子验证")
    @PostMapping("/login/otp/verify")
    @NoNeedLogin
    public ResponseDTO<LoginResultVO> verifyEmployeeOtp(@RequestBody @Valid LoginOtpVerifyForm verifyForm, HttpServletRequest request) {
        String ip = ServletUtil.getClientIP(request);
        String userAgent = ServletUtil.getHeaderIgnoreCase(request, RequestHeaderConst.USER_AGENT);
        return loginService.verifyEmployeeOtp(verifyForm, ip, userAgent);
    }
