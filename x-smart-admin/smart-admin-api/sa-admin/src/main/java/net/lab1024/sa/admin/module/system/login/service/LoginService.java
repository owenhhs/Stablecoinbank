...
import net.lab1024.sa.admin.module.system.employee.manager.EmployeeOtpManager;
import net.lab1024.sa.admin.module.system.employee.service.EmployeeService;
import net.lab1024.sa.admin.module.system.login.domain.*;
...
public class LoginService implements StpInterface {
    ...
    @Resource
    private EmployeeOtpManager employeeOtpManager;
    ...
    public ResponseDTO<LoginResultVO> login(LoginForm loginForm, String ip, String userAgent) {
        ...
        EmployeeOtpEntity employeeOtpEntity = employeeOtpManager.getEmployeeOtp(employeeEntity.getEmployeeId());
        ...
        if (employeeOtpEntity != null && employeeOtpEntity.getEnabledFlag().equals(true)) {
            // 生成临时凭证
            String scrip = RandomStringUtils.randomAlphanumeric(32);
            LoginResultVO loginResultVO = new LoginResultVO();
            loginResultVO.setToken(scrip);

            String redisKey = redisService.generateRedisKey(AdminRedisKeyConst.LOGIN_SCRIP_CACHE, scrip);
            LoginCacheVO loginCacheVO = new LoginCacheVO();
            loginCacheVO.setLoginDevice(loginDeviceEnum.getValue());
            loginCacheVO.setEmployeeId(employeeEntity.getEmployeeId());
            redisService.set(redisKey, loginCacheVO, 60 * 10);

            if (employeeOtpEntity.getBoundFlag().equals(true)) {
                loginResultVO.setOtpTip(2);
            } else {
                loginResultVO.setOtpTip(1);
            }
        ...
        LoginResultVO loginResultVO = loginLast(employeeEntity, ip, userAgent, loginDeviceEnum);
     ...
     * @return
     */
    public ResponseDTO<String> getEmployeeOtpQrcode(String scrip) {
        log.info("getEmployeeOtpQrcode, scrip:{}", scrip);
        String redisKey = redisService.generateRedisKey(AdminRedisKeyConst.LOGIN_SCRIP_CACHE, scrip);
        LoginCacheVO loginCacheVO = redisService.getObject(redisKey, LoginCacheVO.class);
        if (loginCacheVO == null) {
            return ResponseDTO.error(UserErrorCode.OTP_SCRIP_EXPIRED);
        }
        Long employeeId = loginCacheVO.getEmployeeId();

        EmployeeEntity employeeEntity = employeeManager.getById(employeeId);
        if (employeeEntity == null) {
            return ResponseDTO.error(UserErrorCode.DATA_NOT_EXIST);
        }
        EmployeeOtpEntity employeeOtpEntity = employeeOtpManager.getEmployeeOtp(employeeId);
        if (employeeOtpEntity == null || employeeOtpEntity.getEnabledFlag().equals(false)) {
            return ResponseDTO.error(UserErrorCode.OTP_DISABLE);
        }
        if (employeeOtpEntity.getBoundFlag().equals(true)) {
            return ResponseDTO.error(UserErrorCode.OTP_BOUND);
        }
        String qrcode = OtpUtl.getQrcode(employeeEntity.getActualName(), employeeOtpEntity.getSecretKey());
        return ResponseDTO.ok(qrcode);
    }

    public ResponseDTO<String> bindEmployeeOtp(LoginOtpVerifyForm bindForm) {
        String redisKey = redisService.generateRedisKey(AdminRedisKeyConst.LOGIN_SCRIP_CACHE, bindForm.getScrip());
        LoginCacheVO loginCacheVO = redisService.getObject(redisKey, LoginCacheVO.class);
        if (loginCacheVO == null) {
            return ResponseDTO.error(UserErrorCode.OTP_SCRIP_EXPIRED);
        }
        ...
        boolean pass = OtpUtl.checkCode(employeeOtpEntity.getSecretKey(), bindForm.getCode());
        if (!pass) {
            return ResponseDTO.error(otpError("bind", employeeId));
        }
        employeeOtpEntity.setBoundFlag(true);
        employeeOtpManager.updateById(employeeOtpEntity);

        return ResponseDTO.ok();
    }


    public ResponseDTO<LoginResultVO> verifyEmployeeOtp(LoginOtpVerifyForm bindForm, String ip, String userAgent) {
        String redisKey = redisService.generateRedisKey(AdminRedisKeyConst.LOGIN_SCRIP_CACHE, bindForm.getScrip());
        LoginCacheVO loginCacheVO = redisService.getObject(redisKey, LoginCacheVO.class);
        if (loginCacheVO == null) {
            return ResponseDTO.error(UserErrorCode.OTP_SCRIP_EXPIRED);
        ...
        Long employeeId = loginCacheVO.getEmployeeId();
        LoginDeviceEnum loginDeviceEnum = SmartEnumUtil.getEnumByValue(loginCacheVO.getLoginDevice(), LoginDeviceEnum.class);
        ...
        if (employeeOtpEntity.getBoundFlag().equals(false)) {
            return ResponseDTO.error(UserErrorCode.OTP_UNBOUND);
        }
        boolean pass = OtpUtl.checkCode(employeeOtpEntity.getSecretKey(), bindForm.getCode());
        if (!pass) {
            return ResponseDTO.error(otpError("verify", employeeId));
        }
    ...
    public UserErrorCode otpError(String operate, Long employeeId) {
        String redisKey = redisService.generateRedisKey(AdminRedisKeyConst.LOGIN_SCRIP_CACHE, String.format("%s:%s", operate, employeeId));
        long count = redisService.incr(redisKey, 30);
        if (count > 3) {
            return UserErrorCode.OTP_ERROR_FREQUENTLY;
        }
        return UserErrorCode.OTP_CODE_FAIL;
    }


    public LoginResultVO loginLast(EmployeeEntity employeeEntity, String ip, String userAgent, LoginDeviceEnum loginDeviceEnum) {
