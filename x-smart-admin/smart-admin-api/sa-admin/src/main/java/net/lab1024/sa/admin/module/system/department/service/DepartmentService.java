...
public class DepartmentService {
    ...
    public ResponseDTO<String> updateDepartment(DepartmentUpdateForm updateDTO) {
        ...
        queryWrapper.eq(PaymentChannelInfoEntity::getDepartmentId, entity.getDepartmentId());
        PaymentChannelInfoEntity paymentChannelInfoEntity = paymentChannelInfoDao.selectOne(queryWrapper);
        if (paymentChannelInfoEntity != null) {
            paymentChannelInfoEntity.setMerName(updateDTO.getName());
            paymentChannelInfoDao.updateById(paymentChannelInfoEntity);
        }
        // 更新支付渠道父级信息
        LambdaQueryWrapper<PaymentChannelParentEntity> parentQueryWrapper = new LambdaQueryWrapper<>();
        parentQueryWrapper.eq(PaymentChannelParentEntity::getDepartmentId, entity.getDepartmentId());
        PaymentChannelParentEntity paymentChannelParentEntity = paymentChannelParentDao.selectOne(parentQueryWrapper);
        if (paymentChannelParentEntity != null) {
            paymentChannelParentEntity.setMerName(updateDTO.getName());
            paymentChannelParentDao.updateById(paymentChannelParentEntity);
        }
    ...
    public ResponseDTO<List<DepartmentTreeVO>> departmentTree() {
        ...
        List<DepartmentTreeVO> result = treeVOList.stream().map(v -> filterByDepartmentId(v, requestUser.getDepartmentId())).collect(Collectors.toList());
        return ResponseDTO.ok(result);
    }


    public DepartmentTreeVO filterByDepartmentId(DepartmentTreeVO departmentTreeVO, Long departmentId) {
        if (departmentId == null) {
            return departmentTreeVO;
        }
        if (departmentTreeVO.getDepartmentId().equals(departmentId)) {
            return departmentTreeVO;
        }
        if (CollectionUtils.isEmpty(departmentTreeVO.getChildren())) {
            return null;
        }
        List<DepartmentTreeVO> children = new ArrayList<>();
        for (DepartmentTreeVO child : departmentTreeVO.getChildren()) {
            DepartmentTreeVO d = filterByDepartmentId(child, departmentId);
            if (d != null) {
                children.add(d);
            }
        }
        departmentTreeVO.setChildren(children);
        return departmentTreeVO;
    }
     ...
     *
     */
    public List<DepartmentVO> selfAndChildrenList(Long departmentId) {
        List<DepartmentVO> list = queryAllParentDepartment(departmentId);
        List<Long> departmentIdList = departmentCacheManager.getDepartmentSelfAndChildren(departmentId);
