  ...
  <a-form class="smart-query-form">
    <a-row class="smart-query-form-row">
      <a-form-item label="关键字" class="smart-query-form-item">
        <a-input style="width: 240px" v-model:value="queryForm.keyword" placeholder="批次号/订单号/付款人姓名/子商户名称" />
      </a-form-item>
  ...
  <a-card size="small" :bordered="false" :hoverable="true">
    ...
    <a-row class="smart-table-btn-block">
      <div class="smart-table-setting-block">
        <!-- <a-button @click="importOrders" type="primary" size="small" v-privilege="'cash-order:list:import'">
          <template #icon>
            <ImportOutlined />
          </template>
          导入订单
        </a-button> -->
        <UploadExcel :on-success="importOrders" :before-upload="beforeUpload" v-privilege="'cash-order:list:import'">
          <template #icon>
            <ImportOutlined />
          </template>
          导入订单
      ...
      <div class="smart-table-setting-block">
        <TableOperator v-model="columns" :tableId="TABLE_ID_CONST.BUSINESS.ERP.CASH_ORDER_IMPORT" :refresh="queryData" />
      </div>
    </a-row>
    <!---------- 表格操作行 end ----------->
...
<script setup>
  ...
  import OrderInfoForm from './cash-order-import-info-form.vue';
  ...
  async function queryData() {
    ...
    try {
      let queryResult = await cashOrderInfoApi.manualImportQueryPage(queryForm);
  ...
  // 导入订单
  async function importOrders({ header, results }) {
    ...
    try {
      SmartLoading.show();
      await cashOrderInfoApi.manualImportExcel(arr);
    } catch (e) {
      smartSentry.captureError(e);
    } finally {
      SmartLoading.hide();
