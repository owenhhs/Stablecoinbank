...
-->
<template>
    <a-drawer title="渠道支付方式配置" width="90%" :open="visibleFlag" :maskClosable="false" :destroyOnClose="true" @close="onClose">
            ...
            <a-row class="smart-query-form-row">
                ...
                <a-form-item label="状态" class="smart-query-form-item">
                    <a-select v-model:value="queryForm.status" style="width: 100px" placeholder="请选择" allow-clear>
                        <a-select-option class="pay-status" v-for="item in channelInfoConst.STATUS_LIST" :key="item.value" :value="item.value">{{ item.desc }}</a-select-option>
                    </a-select>
                </a-form-item>
        ...
        <a-card size="small" :bordered="false" :hoverable="true">
            ...
            <a-row class="smart-table-btn-block">
                ...
                <div class="smart-table-setting-block">
                    <TableOperator v-model="columns" :tableId="TABLE_ID_CONST.BUSINESS.ERP.PAYMENT_METHOD_CFG" :refresh="queryData" />
                </div>
            </a-row>
            <!---------- 表格操作行 end ----------->
            ...
            <PaymentMethodCfgForm  ref="formRef" @reloadList="queryData"/>
...
<script setup>
    ...
    import PaymentMethodCfgForm from './payment-method-cfg-form.vue';
    ...
    async function queryData() {
        ...
        try {
            let queryResult = await paymentMethodCfgApi.queryPage(queryForm);
    ...
    async function requestDelete(data){
        ...
        try {
            ...
            await paymentMethodCfgApi.delete(data.id);
    ...
    async function requestBatchDelete() {
        try {
            SmartLoading.show();
            await paymentMethodCfgApi.batchDelete(selectedRowKeyList.value);
            message.success('删除成功');
            queryData();
        } catch (e) {
            smartSentry.captureError(e);
