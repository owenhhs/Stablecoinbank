...
public final class SmartExcelUtil {
    ...
    public static void exportExcel(HttpServletResponse response, String fileName, String sheetName, Class head, Collection<?> data) throws IOException {
        ...
        // 设置下载消息头
        SmartResponseUtil.setDownloadFileHeader(response, fileName, fileSize);
     ...
     * 文件分块写入 导出
     */
    public static void exportExcelBlockWithWatermark(HttpServletResponse response, String fileName, String sheetName, Class head, SmartExcelDataHandler handler, String watermarkString) {
        // 1. 创建Excel写入对象（默认会自动关闭资源）
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        try (ExcelWriter excelWriter = EasyExcel.write(bos, head).build()) {
            // 2. 创建Sheet对象（可指定Sheet名称和索引）
            ExcelWriterSheetBuilder sheetBuilder = EasyExcel.writerSheet(sheetName);
            if (StringUtils.isNotEmpty(watermarkString)) {
                // 水印
                Watermark watermark = new Watermark(watermarkString);
                sheetBuilder.registerWriteHandler(new CustomWaterMarkHandler(watermark));
            }
            WriteSheet writeSheet = sheetBuilder.build();
            Long page = 1L;
            List<?> data;
            while (CollectionUtils.isNotEmpty(data = handler.next(page))) {
                List<? extends List<?>> partition = Lists.partition(data, 1000);
                for (List<?> row : partition) {
            ...
            log.error("exportExcelBlockWithWatermark, {}", e.getMessage(), e);
...
//            }
            ...
            try {
                // 添加水印的具体操作
                addWatermarkToSheet(xssfWorkbook, bufferedImage);
            } catch (Exception e) {
                log.error("添加水印出错:", e);
            }
        }
        ...
        private XSSFWorkbook getXSSFWorkbook(Workbook workbook) {
            if (workbook instanceof SXSSFWorkbook) {
                ...
                log.error("不支持的Workbook类型: {}", workbook.getClass().getName());
                return null;
            }
        }
    ...
    private static class Watermark {
         ...
         * 字体样式
         */
        private Font font = new Font("GenShinGothic-Regular", Font.BOLD, 26);
        ...
        private Font getFont() {
            try (InputStream is = Watermark.class
                    .getResourceAsStream("/fonts/SourceHanSansCN-Regular.ttf");
                 BufferedInputStream bis = new BufferedInputStream(is)) {
                return Font.createFont(Font.TRUETYPE_FONT, bis)
                        .deriveFont(Font.BOLD, 26);
            } catch (Exception e) {
                return new Font("GenShinGothic-Regular", Font.BOLD, 26);
            }
        }
    }
}
