...
public class FileService {
     ...
     * @return
     */
    public ResponseDTO<FileUploadVO> fileUploadLocalFile(File file, Integer folderType) {
        FileFolderTypeEnum folderTypeEnum = SmartEnumUtil.getEnumByValue(folderType, FileFolderTypeEnum.class);
        if (null == folderTypeEnum) {
            return ResponseDTO.userErrorParam("文件夹错误");
        }
        ...
        // 进行上传
        ResponseDTO<FileUploadVO> response = fileStorageService.uploadFile(file, folderTypeEnum.getFolder());
        if (!response.getOk()) {
            return response;
        }
     ...
     * @return
     */
    public ResponseDTO<String> getFileUrlByFileId(Long fileId) {
        FileEntity file = fileDao.selectById(fileId);
        if (file == null) {
            return ResponseDTO.error(OpenErrorCode.DATA_NOT_EXIST);
        }
        List<String> fileKeyArray = Collections.singletonList(file.getFileKey());
        List<String> fileUrlList = Lists.newArrayListWithCapacity(fileKeyArray.size());
        for (String fileKey : fileKeyArray) {
            ResponseDTO<String> fileUrlResponse = fileStorageService.getFileUrl(fileKey);
            if (fileUrlResponse.getOk()) {
     ...
     * 根据文件服务类型 和 FileKey 下载文件
     */
    public ResponseDTO<FileDownloadVO> getDownloadFile(Long fileId, String userAgent) {
        FileVO fileVO = fileDao.getByFileId(fileId);
        if (fileVO == null) {
            return ResponseDTO.userErrorParam("文件不存在");
        }

        // 根据文件服务类 获取对应文件服务 查询 url
        ResponseDTO<FileDownloadVO> download = fileStorageService.download(fileVO.getFileKey());
        if(download.getOk()){
            download.getData().getMetadata().setFileName(fileVO.getFileName());
        }
        return download;
