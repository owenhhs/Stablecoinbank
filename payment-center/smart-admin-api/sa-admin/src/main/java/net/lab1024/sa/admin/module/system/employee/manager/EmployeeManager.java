...
public class EmployeeManager extends ServiceImpl<EmployeeDao, EmployeeEntity> {
    ...
    public void saveEmployee(EmployeeEntity employee, List<Long> roleIdList, Boolean enabledOtpFlag) {
        // 保存员工 获得id
        employeeDao.insert(employee);

        // 保存员工otp
        employeeOtpManager.saveEmployeeOtp(employee.getEmployeeId(), enabledOtpFlag);
     ...
     */
    @Transactional(rollbackFor = Throwable.class)
    public void updateEmployee(EmployeeEntity employee, List<Long> roleIdList, Boolean enabledFlag) {
        // 保存员工 获得id
        employeeDao.updateById(employee);

        // 查询otp信息
        EmployeeOtpEntity employeeOtpEntity = employeeOtpManager.getEmployeeOtp(employee.getEmployeeId());
        if (employeeOtpEntity == null) {
            employeeOtpManager.saveEmployeeOtp(employee.getEmployeeId(), enabledFlag);
        } else if (!employeeOtpEntity.getEnabledFlag().equals(enabledFlag)) {
            employeeOtpEntity.setEnabledFlag(enabledFlag);
